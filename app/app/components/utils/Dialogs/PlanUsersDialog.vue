...
async function getPlanUsers() {
  try {
    ...
    const response = await $api.plan.getPlanUsers(filters.value)
...
<template>
  <LazyUtilsDialogsBaseDialog :dialog-id="`plan${props.plan.id}`" v-model="isRendering">
    <template #title>
      <span class="!text-base">کاربرانی که این پلن را همراهی میکنند</span>
    </template>
    <template #default>
      <div v-if="props.plan" class="w-full flex flex-col justify-start items-start gap-4">
        ...
        <LazyUtilsEmptyView v-else
                            empty-text="در حال حاضر کاربر فعالی این برنامه را همراهی نمیکند"></LazyUtilsEmptyView>
