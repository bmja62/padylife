...
import {ICreateOrUpdateExerciseCategoryPayload} from "@/services/ExerciseService";
...
const exerciseCategoryPayload = ref<ICreateOrUpdateExerciseCategoryPayload>({
  name: null,
})
...
async function createExerciseCategory() {
  try {
    ...
    const response = await $api?.exercise.createExerciseCategory(exerciseCategoryPayload.value)
    if (response.data.isSuccess) {

      exerciseCategoryPayload.value = {
        name: null,
      }
...
<template>
  ...
  >
    ...
    >
      <VCol cols="12">
        <VTextField
          v-model.trim="exerciseCategoryPayload.name"
          variant="outlined"
          density="compact"
          label="نام دسته بندی"
          color="success"
