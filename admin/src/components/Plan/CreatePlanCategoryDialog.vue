...
import {ICreatePlanCategory} from "@/services/PlanService";
...
const planCategoryPayload = ref<ICreatePlanCategory>({
  name: null,
})
...
// Functions
async function createPlanCategory() {
  try {
    spinner.showSpinner()

    const response = await $api?.plan.createPlanCategory(planCategoryPayload.value)
    if (response.data.isSuccess) {
      planCategoryPayload.value = {
        name: null,
      }
...
<template>
  ...
  >
    ...
    >
      <VCol cols="12">
        <VTextField
          v-model.trim="planCategoryPayload.name"
          variant="outlined"
          density="compact"
          label="نام دسته بندی"
          color="success"
