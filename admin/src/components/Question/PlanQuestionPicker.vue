...
async function getAllQuestions() {
  try {
    const response = await $api?.plan.getPlanQuestions(questionsListFilters.value)
    questionsList.value = response?.data.data.data
    if (props.filterIsMain) {
      questionsList.value = questionsList.value.filter(e => !e.isMain)
    }
...
<template>
  <CustomPicker
    ...
    :item-value="(item: any) => item.planQuestionId"
