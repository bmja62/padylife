...
import {IZone} from "@/services/WarehouseService";
...
async function deleteWarehouseZone() {
  if (props.selectedItem) {
    try {
      spinner.showSpinner()

      const response = await $api?.warehouse.removeWarehouseZone(props.selectedItem.id)
...
<template>
  <CustomDeleteDialog
    ...
    @delete="deleteWarehouseZone"
  >
    <VCol cols="12">
      <p>
        آیا از حذف این منطقه
        اطمینان دارید؟
      </p>
    </VCol>
  </CustomDeleteDialog>
