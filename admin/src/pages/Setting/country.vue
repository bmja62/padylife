...
import CreateCountryDialog from "@/components/Countries/CreateCountryDialog.vue";
import DeleteCountryDialog from "@/components/Countries/DeleteCountryDialog.vue";
import UpdateCountryDialog from "@/components/Countries/UpdateCountryDialog.vue";
...
const tableHeaders: VDataTable['headers'] = [
  {title: 'شناسه', key: 'id'},
  {title: 'نام انگلیسی کشور', key: 'countryName'},
  {title: 'نام فارسی کشور', key: 'countryNameFa'},
  {title: 'وضعیت', key: 'isActive'},
  // {title: 'نوع ویژگی', key: 'type', value: (item: IEntity) => productAttributeTypesShow[item.type]},
...
async function getEntity() {
  try {
    spinner.showSpinner()
    const response = await $api?.countries.getAll(entityFilters.value)
