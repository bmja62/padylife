...
const tableHeaders: VDataTable['headers'] = [
  {title: 'شناسه', key: 'productCategoryId'},
  {title: 'نام', key: 'name'},
...
async function getSeoData() {
  try {
    spinner.showSpinner()
    const response = await $api?.products.getAllProductCategory(seoDataFilters.value)
    seoDataList.value = response?.data.productCategories as Array<ItempSeoData>
    totalCount.value = response?.data.categoriesCount as number
  } catch (error) {
    console.error(error)
  } finally {
    spinner.hideSpinner()
...
async function updateSeoData() {
  try {
    spinner.showSpinner()
    const response = await $api?.products.updateProductCategorySeo(tempSeoData.value)
...
<template>
  ...
  >
    <template #title>
      بروزرسانی سئوی دسته بندی محصولات
    </template>
    <template #filters>
      <VCol
        cols="12"
      ...
      >
        <VTextField
          v-model="seoDataFilters.searchCommand"
          label="جستجو..."
          hide-details
