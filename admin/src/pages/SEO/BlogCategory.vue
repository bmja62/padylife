...
const tableHeaders: VDataTable['headers'] = [
  {title: 'شناسه', key: 'blogCategoryId'},
  {title: 'نام', key: 'name'},
...
async function getSeoData() {
  try {
    spinner.showSpinner()
    const response = await $api?.blog.getAllBlogCategories(seoDataFilters.value)
    seoDataList.value = response.data as Array<ItempSeoData>
    totalCount.value = response.data.length as number
  } catch (error) {
    console.error(error)
  } finally {
    spinner.hideSpinner()
...
async function updateSeoData() {
  try {
    spinner.showSpinner()
    const response = await $api?.blog.updateBlogCategorySEO(tempSeoData.value)
...
<template>
  ...
  >
    <template #title>
      بروزرسانی سئوی دسته بندی اخبار
    </template>
    <template #filters>
      <VCol
        cols="12"
