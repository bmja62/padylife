...
export function useUtils() {
  ...
  function adjustDate(inputDate, days, operation: 'add' | 'subtract') {
    // Create a new Date object to avoid modifying the original date
    const resultDate = new Date(inputDate);

    // Adjust the date based on the operation
    if (operation.toLowerCase() === 'subtract' || operation === '-') {
  ...
  async function uploadAnyFile(file: File, fileType: UploaderTypes) {
    ...
    try {
      ...
      myForm.append('FileType', fileType)
